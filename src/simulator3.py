...
class Simulator3:
    ...
    def schedule_new_system_arrival(self,):
        ...
        self.insert_event(ARRIVAL, 1, self.__current_timestamp + arrival_time, self.get_client_counter())
    
    def insert_event(self, event_type, event_queue, event_timestamp, event_client):
    ...
    def remove_current_service_system_departure_event(self,):
        ...
        self.__event_q = list(filter(lambda x: not(x['event_type'] == DEPARTURE and x['event_queue'] == 2 and x['event_client'] == service_client), self.__event_q))
    ...
    def handle_event(self, event):
        ...
        if(event_type == ARRIVAL and event_queue == 1):
            ...
            if(self.__current_service == None):
                ...
                self.insert_event(DEPARTURE, 1, self.__current_timestamp + service_time, event_client)
            else:
                service_client = self.__current_service['client']
            ...
            if(len(self.__waiting_qs[0]) == 0 and self.__current_service == None):
            ...
            if(len(self.__waiting_qs[0]) > 0):
                ...
                self.insert_event(DEPARTURE, 1, self.__current_timestamp + service_time, next_client_id)
            
            self.insert_event(ARRIVAL, 2, self.__current_timestamp, event_client)
