...
class Simulator:
    ...
    def __enqueue_in_waiting_q(self,
                               ...
                               in_the_front=False):
        if (in_the_front):
            self.__waiting_qs[queue_number - 1].insert(0, (client_id, remaining_service_time))
        else:
            self.__waiting_qs[queue_number - 1].append((client_id, remaining_service_time))
    ...
    def __set_current_service(self, client_id: int, source_event_type: EventType):
    ...
    def __handle_end_service_1(self, event: Event):
        ...
        if (self.__get_waiting_q_size(1) < 1
            ...
            next_event = Event(client_id=next_client_id, type=EventType.START_SERVICE_1, timestamp=self.__current_timestamp, queue_number=1)

            self.__enqueue_event(next_event, True)
        elif (self.__get_waiting_q_size(2) > 0):
            next_client_id, _ = self.__get_next_client_in_waiting_q(2)

            next_event = Event(client_id=next_client_id, type=EventType.START_SERVICE_2, timestamp=self.__current_timestamp, queue_number=2)

            self.__enqueue_event(next_event, True)

    def __handle_start_service_2(self, event: Event):
        next_client, remaining_service_time = self.__dequeue_from_waiting_q(event.queue_number)
    ...
    def __handle_halt_service_2(self, event: Event):
        ...
        self.__enqueue_in_waiting_q(client_id, 2, in_the_front=True, remaining_service_time=remaining_service_time)
